...
{
    // Cache bean property descriptors as soon as they are fetched. The cache is only
    ...
    private static Map<String, PropertyDescriptor> getpropmap(Class clazz, boolean incl_cls)
    {
        ...
        if (propmap == null)
        {
            try
            {
                ...
                bean_info.getPropertyDescriptors();
...
//                cache.put(clazz, propmap);
    ...
    /**
     * @param beanClass The bean class whose property descriptors are desired.
    ...
    /**
     * @param bean The bean whose property descriptors are desired. Cannot be
     * <code>null</code>
     *
     * @return a <code>Map</code> of all property descriptors in the bean. The
     * map elements are keyed by property name.
    ...
    /**
     * @param bean Any object conforming to the Java Beans Specification
     * @param propname The name a bean property.
     * @return The <code>PropertyDescriptor</code> for that property as returned
     * from the <code>BeanInfo</code> associated with that bean. If there is
     * no property with that name in the bean, <code>null</code> is returned.
     */
    public static PropertyDescriptor getPropertyDescriptor(Object bean,
    String propname)
    {
        return (PropertyDescriptor)getpropmap(bean.getClass()).get(propname);
    }
    ...
    public static Object getProperty(Object bean, PropertyDescriptor prop, int index)
    {
        try
        {
            ...
            if (prop instanceof IndexedPropertyDescriptor)
                if (index < 0)
                    ...
                    method = ((IndexedPropertyDescriptor)prop).getIndexedReadMethod();
            else if (index >= 0)
                throw new java.lang.UnsupportedOperationException(
                "Property " + prop.getName() + " of bean " + bean.getClass().getName() +
        ...
        {
            throw new HGException("Illegal access to property " + prop.getName() +
        ...
        catch (InvocationTargetException ex)
        {
            ...
            "property " + prop.getName() + " of bean " + bean.getClass().getName() +
            ex.toString() + ", target exception is " + ex.getTargetException().toString(), ex);
        }
    }
     ...
     *
     * @param bean An object conforming to the Java Beans Specification.
     * @param propname The name of the desired bean property.
     * @param index The <code>integer</code> index within the property. If the value
     * is negative, the property itself is returned. That is, for example, if
     * the property is <code>String [] values</code> and if <code>index</code>
     * is < 0, <code>values</code> will be returned, otherwise <code>values[index]</code>
     * will be returned.
     * @return The value of the property at the specified index.
     *
     * @throws HGException if the property cannot be found in the bean or if
    ...
    {
        return getProperty(bean, getorthrow_propdesc(bean, propname, index), index);
    }
    ...
    /**
     * Same as <code>setProperty(Object, String, int, Object)</code>, but use a supplied
     * <code>PropertyDescriptor</code> instead of the property name.
     */
    ...
    Object newvalue)
    {
        try
        {
            ...
            if (prop instanceof IndexedPropertyDescriptor)
                if (index < 0)
                    ...
                    method = ((IndexedPropertyDescriptor)prop).getIndexedWriteMethod();
            else if (index >= 0)
                throw new java.lang.UnsupportedOperationException(
                "Property " + prop.getName() + " of bean " + bean.getClass().getName() +
                " is not an indexed property.");
        ...
        catch (InvocationTargetException ex)
        {
            throw new HGException("InvocationTargetException while accessing " +
            "property " + prop.getName() + " of bean " + bean.getClass().getName() +
            ex.toString() + ", taget exception is " + ex.getTargetException().toString(), ex);
        }
    }
    
    /**
     * Same as <code>setProperty(Object, String, Object)</code>, but use a supplied
     * <code>PropertyDescriptor</code> instead of the property name.
     */
    public static void setProperty(Object bean,
    PropertyDescriptor prop,
     ...
     * @param propname The name of the desired bean property.
     * @param index The <code>integer</code> index within the property. If the value
     * is negative, the property itself will be modified. That is, for example, if
     * the property is <code>String [] values</code> and if <code>index</code>
     * is < 0, <code>values</code> will be modified, otherwise <code>values[index]</code>
     * will be modified.
     * @param newvalue The value to assign to the specified property.
     *
     * @throws HGException if the property cannot be found in the bean or if
     * <code>index >= 0 </code> and the property is not an indexed property.
     */
     ...
     *
     * @param classname The fully qualified class name of the bean to construct.
     * @return The newly constructed bean.
     * @throws HGException is there's any error during the construction process,
     * such as ClassNotFoundException etc.
     */
