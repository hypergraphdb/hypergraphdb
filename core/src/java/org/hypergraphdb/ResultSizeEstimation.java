...
class ResultSizeEstimation
{
	...
	static long countResultSet(HGQuery<?> q)
	{
		...
		HGSearchResult<HGPersistentHandle> rs = (HGSearchResult<HGPersistentHandle>) q
				.execute();
		try
		{
			long result = 0;
	...
	static
	{
		...
		countersMap.put(AnyAtomCondition.class, new Counter()
		{
			...
			{
				HGSearchResult<HGPersistentHandle> rs = graph.indexByType
						.scanKeys();
				try
				{
					long result = 0;
					while (rs.hasNext())
						// TODO: this is actually stupid because that we have to
						// create another cursor
						// and position it on the same key as our
						// rs.current...but can't break
		...
		countersMap.put(AtomTypeCondition.class, new Counter()
		{
			public long count(HyperGraph graph, HGQueryCondition x)
			{
				...
				if (typeHandle == null)
					typeHandle = graph.getTypeSystem().getTypeHandle(
							cond.getJavaClass());
				return graph.indexByType.count(graph
						.getPersistentHandle(typeHandle));
			}
		...
		countersMap.put(TypePlusCondition.class, new Counter()
		{
			public long count(HyperGraph graph, HGQueryCondition x)
			{
				...
				for (HGHandle h : cond.getSubTypes(graph))
					result += graph.indexByType.count(graph
		...
		countersMap.put(TypedValueCondition.class, new Counter()
		{
			public long count(HyperGraph graph, HGQueryCondition x)
			{
				...
				if (type instanceof HGSearchable
						&& cond.getOperator() == ComparisonOperator.EQ)
				{
					HGSearchResult<HGPersistentHandle> rs = ((HGSearchable) type)
							.find(cond.getValue());
					try
					{
						long result = 0;
			...
			public long count(HyperGraph graph, HGQueryCondition x)
			{
				...
				if (value == null)
					throw new HGException(
							"Count by null values is not supported yet.");
				HGHandle type = graph.getTypeSystem().getTypeHandle(value);
				return countersMap.get(TypedValueCondition.class).count(
						graph,
						new TypedValueCondition(type, vc.getValue(), vc
								.getOperator()));
			}
			...
			public long cost(HyperGraph graph, HGQueryCondition x)
			{
				...
				if (value == null)
					throw new HGException(
							"Count by null values is not supported yet.");
				HGHandle type = graph.getTypeSystem().getTypeHandle(value);
				return countersMap.get(TypedValueCondition.class).cost(
						graph,
						new TypedValueCondition(type, vc.getValue(), vc
								.getOperator()));
			}
		...
		countersMap.put(IndexCondition.class, new Counter()
		{
			...
			{
				IndexCondition ic = (IndexCondition) x;
				return ic.getOperator() == ComparisonOperator.EQ ? 1
						: Integer.MAX_VALUE;
			}
		});
		...
		countersMap.put(IndexedPartCondition.class, new Counter()
		{
			public long count(HyperGraph graph, HGQueryCondition x)
			{
				...
				if (ip.getOperator() == ComparisonOperator.EQ)
					return ((HGIndex<Object, Object>) ip.getIndex()).count(ip
							.getPartValue());
				else
			...
			{
				IndexedPartCondition ip = (IndexedPartCondition) x;
				return ip.getOperator() == ComparisonOperator.EQ ? 1
						: Integer.MAX_VALUE;
			}
		});
		...
		countersMap.put(AtomPartCondition.class, new Counter()
		{
			...
			{
				throw new HGException(
						"Can't count AtomPartCondition results: this condition can't be used alone.");
			}

			public long cost(HyperGraph graph, HGQueryCondition x)
			{
				throw new HGException(
						"Can't estimate cost of counting AtomPartCondition results: this condition can't be used alone.");
			}
		});
		...
		countersMap.put(And.class, new Counter()
		{
			public long count(HyperGraph graph, HGQueryCondition x)
			{
				ExpressionBasedQuery q = (ExpressionBasedQuery) HGQuery.make(
						graph, x);
				x = q.getCondition();
		...
		countersMap.put(Or.class, new Counter()
		{
			public long count(HyperGraph graph, HGQueryCondition x)
			{
				...
				for (HGQueryCondition cond : (Or) x)
				{
					...
					result += c == null ? countResultSet(graph, cond) : c
							.count(graph, cond);
				}
				return result;
			}
