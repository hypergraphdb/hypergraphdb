...
* 
* Adapter by Kobrix Software for purposes of HyperGraphDB. The modifications
* involve changes to the compareTo method, to always perform byte to byte
* comparison in line with the underlying storage mechanism.
*/
...
//       }
       ...
       byte [] thatId = other.mId;
       for (int i = 0; i < 16; ++i) {
           int cmp = (((int) mId[i]) & 0xFF) - (((int) thatId[i]) & 0xFF);
           if (cmp != 0) {
               return cmp;
           }
       }
