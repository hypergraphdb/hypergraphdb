	...
	public HGHandle add(Object atom, int flags)
	{
		...
		if (atom instanceof HGLink)
		{
			...
			if (eventManager.dispatch(this,
					 new HGAtomProposeEvent(atom, type, flags, "HyperGraph.add")) == HGListener.Result.cancel)
				return null;
...
//
	...
	/**
	 * <p>Checks if the handle exist in the database.</p>
	 *
	 * @param handle The <code>HGHandle</code> of the atom whose existence is being verified.
...
//				removeTransaction(h, false);
	 ...
	 * already exists with this handle, an exception will be thrown. This parameter cannot be <code>null</code>.
						 ...
						 final Object instance)
	{
		...
		{ public Object call() {
			...
			if (exist(atomHandle))
				throw new HGException("Can't define an existing atom. Use replace instead. Hdl: " + atomHandle);
										 ...
										 final int flags)
	{
		...
		{ public Object call() {
				if (eventManager.dispatch(HyperGraph.this,
						 new HGAtomProposeEvent(instance, typeHandle, flags, "HyperGraph.define")) == HGListener.Result.cancel)
					return null;
				...
				eventManager.dispatch(HyperGraph.this, new HGAtomAddedEvent(atomHandle, "HyperGraph.define"));
				return null;
		}});
	}
...
//    	TypeUtils.atomConstructionComplete(HyperGraph.this, layout[1]);
									...
									final HGHandle typeHandle)
	{
		...
		else
		{
			...
			for (int i = 2; i < layout.length; i++)  //takes care of replacing a link by a non link
				removeFromIncidenceSet(layout[i], pHandle);
		}
		...
		if (lHandle == null)
		{
			...
			HGAtomAttrib attribs = config.isUseSystemAtomAttributes() ? getAtomAttributes(pHandle) : new HGAtomAttrib();
