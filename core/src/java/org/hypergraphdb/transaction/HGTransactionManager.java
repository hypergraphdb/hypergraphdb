...
public class HGTransactionManager
{
	...
	/**
	 * <p>
	 * Enable or disable transaction. Note that all current transactions will be
	 * silently aborted so make sure any pending transactions are completed
			...
			HGTransactionConfig config)
	{
		...
		{
			if (getContext().getCurrent().isReadOnly() && !config.isReadonly())
			{
	...
	private boolean handleTxException(Throwable t, HGTransactionConfig config)
	{
		boolean retry = false;
		...
		for (Throwable cause = t; cause != null; cause = cause.getCause())
		{
			...
			else if (cause instanceof TransactionIsReadonlyException
	...
	public <V> V transact(Callable<V> transaction, HGTransactionConfig config)
	{
		...
		while (true)
		{
			...
			catch (Throwable t)
			{
				...
				if (handleTxException(t, config)) // will re-throw if we can't
			...
			{
				endTransaction(true);
				successful.incrementAndGet(); // "successful" means not
