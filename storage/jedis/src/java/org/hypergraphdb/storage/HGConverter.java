...
public class HGConverter
{
	...
	try 						
	{	
		// return IOUtils.toByteArray(new StringReader(s), Protocol.CHARSET);	
		return s.getBytes(Protocol.CHARSET);
	}
	catch (IOException ioEx) 	{throw new HGException("Error when converting String to Byte" + ioEx);} 
	}
    ...
    public static String convertByteArraytoString(byte [] b)
    {
    	try 						
    	{
    		...
    		// return IOUtils.toString(new ByteArrayInputStream(b), Protocol.CHARSET);	
    	}

    	catch (IOException ioEx) 	{throw new HGException("Error when converting from Byte to String " + ioEx);} 
	}
	...
	public static String convertHandleArrayToString (HGPersistentHandle[] link, int handleSize)
	    {
		  ...
		  try {
	    		...
	    		for (int i = 0; i < link.length; i++)
	           {
	    		...
	    		resultstring = new String(buffer, Protocol.CHARSET);
