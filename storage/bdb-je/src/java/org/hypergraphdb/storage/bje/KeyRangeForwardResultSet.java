...
class KeyRangeForwardResultSet<T> extends IndexResultSet<T>
{
	...
	public KeyRangeForwardResultSet(BJETxCursor cursor, 
									DatabaseEntry key,
								    ByteArrayConverter<T> converter)
	{
		super(cursor, 
			  key, 
			  converter,
//			  bje.nextDataSupplier(cursor.cursor(), null, converter),
//			  bje.prevDataSupplier(cursor.cursor(), new DatabaseEntry(key.getData().clone()), converter),
			  null, null,
			  true);		
	}
/*
