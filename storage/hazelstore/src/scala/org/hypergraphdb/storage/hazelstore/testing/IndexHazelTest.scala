                                  ...
                                  val valFilter: Boolean = random.nextBoolean ) extends HazelTest
{
  ...
  val mutations =
    Seq(
      ("addEntry",  testData, ((dataSeq:Seq[Data]) => dataSeq.foreach{ case (key, valueList) => valueList.foreach(tested.addEntry(key, _))})),
      ("removeEntry",  removeData, ((dataSeq:Seq[(K,Seq[V])]) => dataSeq.foreach{ case (key, valueList) => valueList.foreach(tested.removeEntry(key, _))})),
      ("removeAllEntries",testData, ((dataSeq:Seq[(K,Seq[V])]) => dataSeq.foreach{ case (key, _) => tested.removeAllEntries(key)}))
  )
