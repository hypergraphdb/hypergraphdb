...
class HazelBidirecIndex12[K, V] (val name: String,
    ...
    if(key !=null && value != null)
      {
        ...
        execute(new BiAddEntry(keyMapName, kvmmName,vkmmName, valMapName,firstValMapName,valueCountMapName,indexKeyCountName,keyHash,keyBA, valHash, valBA,hstoreConf.getUseTransactionalCallables, hstoreConf.getUseHCIndexing,timeout), false)
      }
    else
      Unit
  }
  ...
  def removeEntry(key: K, value: V){
    if(key !=null && value != null)
      {
        val (keyBA, keyHash, valBA,valHash) = initialVals(key, Option(value))
        execute(new BiRemoveEntry(keyMapName, kvmmName,vkmmName, valMapName,firstValMapName,valueCountMapName,indexKeyCountName,keyHash,keyBA, valHash, valBA, hstoreConf.getUseTransactionalCallables,timeout), false)
      }
    else
      Unit
  }
    ...
    if(key !=null )
      {
        ...
        groupByMember.foreach{ case (member, keysOffThatMember) => clusterExecutor.execute(new DistributedTask(new RemoveAllOnMember(valMapName, vkmmName,keysOffThatMember,hstoreConf.getUseTransactionalCallables),member))}    //parallelized
        kvmm.remove(keyHash)
      ...
      {case (member, keysOfThatMember) =>
        {
          ...
          clusterExecutor.execute(a)
           a
          //val b:Future[java.util.List[ComparableBAW]] = clusterExecutor.submit(a).asInstanceOf[Future[java.util.List[ComparableBAW]]]
          //val b:Future[List[ComparableBAW]] = clusterExecutor.submit(a).asInstanceOf[Future[List[ComparableBAW]]]
          //b
        }
      }
