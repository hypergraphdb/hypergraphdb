...
import org.hypergraphdb.storage.mdbx.type.util.FastOutputStream;
import org.hypergraphdb.storage.mdbx.type.util.PackedInteger;
import org.hypergraphdb.storage.mdbx.type.util.UtfOps;

public abstract class DataOutput extends FastOutputStream
		implements HGDataOutput
{
	/**
	 ...
	 * unsigned byte value. Writes values that can be read using
	 * {@link TupleInput#readBytes}.
	 * 
	 * @param val
	 *            is the string containing the values to be written. Only
	 *            characters with values below 0x100 may be written using this
	 *            method, since the high-order 8 bits of all characters are
	 *            discarded.
	 * 
	 ...
	 * to a two byte unsigned value. Writes values that can be read using
	 * {@link TupleInput#readChars}.
	 * 
	 * @param val
	...
	/**
	 * Writes the specified characters to the buffer, converting each character
	 * to UTF format, and adding a null terminator byte. Writes values that can
	 * be read using {@link TupleInput#readString()}.
	 * 
	 * @param val
	 *            is the string containing the characters to be written.
	 * 
...
//	@Override
	...
	/**
	 * Writes an unsorted float (four byte) value to the buffer. Writes values
...
//	@Override
	...
	/**
	 * Writes an unsorted double (eight byte) value to the buffer. Writes values
...
//	@Override
	...
	/**
	 * Writes a sorted float (four byte) value to the buffer. Writes values that
	 * can be read using {@link TupleInput#readSortedFloat}.
	 * 
	 * @param val
	 *            is the value to write to the buffer.
	 * 
...
//	@Override
	public HGDataOutput writeSortedFloats(float[] val)
	{
		writeRawVarint32(val.length);
		for (int i = 0; i < val.length; i++)
		{
			writeSortedFloat(val[i]);
		}
		return this;
	}

	/**
	 * Writes a sorted double (eight byte) value to the buffer. Writes values
	 * that can be read using {@link TupleInput#readSortedDouble}.
	 * 
	 * @param val
	 *            is the value to write to the buffer.
	 * 
...
//	@Override
	public HGDataOutput writeSortedDoubles(double[] val)
	{
		writeRawVarint32(val.length);
		for (int i = 0; i < val.length; i++)
		{
			writeSortedDouble(val[i]);
		}
		return this;
	}
	 ...
	 * 
	 * @param chars
	 *            is the array of values to be written. Only characters with
	 *            values below 0x100 may be written using this method, since the
	 *            high-order 8 bits of all characters are discarded.
	 * 
	 * @return this tuple output object.
	 * 
	 * @throws NullPointerException
	...
	/**
	 * Writes the specified characters to the buffer, converting each character
	 * to UTF format. Writes values that can be read using
	 * {@link TupleInput#readString(int)} or
	 * {@link TupleInput#readString(char[])}.
	 * 
	 * @param chars
	 *            is the array of characters to be written.
	 * 
	...
	public HGDataOutput writeString(char[] chars)
	{
		if (chars.length == 0)
		{
			writeRawVarint32(1); // 0 is for null, so always add 1 more
			return this;
		}

		int utfLength = UtfOps.getByteLength(chars);
		// writing length instead of using a trailing null byte since won't work
		// with encryption. See:
		// https://stackoverflow.com/questions/22969876/null-character-in-encrypted-data
		writeRawVarint32(utfLength + 1); // 0 is for null, so always add 1 more
		makeSpace(utfLength);
	 ...
	 *             if the byte array representation of val is larger than 0x7fff
	 *             bytes.
	 * 
	 * @see <a href="package-summary.html#integerFormats">Integer Formats</a>
	 */
	...
	{
		return 2 /* length bytes */ + (val.bitLength() + 1 /* sign bit */
				+ 7 /* round up */) / 8;
	}
	...
	{
		/*
		 * The byte format for a BigDecimal value is: Byte 0 ~ L: The scale part
		 * written as a PackedInteger. Byte L+1 ~ M: The length of the unscaled
		 * value written as a PackedInteger. Byte M+1 ~ N: The
	 ...
	 * {@code BigDecimal} value if {@link TupleOutput#writeBigDecimal} were
	 * called.
	 * 
	 * @see <a href="package-summary.html#bigDecimalFormats">BigDecimal
	 *      Formats</a>
	...
	public HGDataOutput writeSortedBigDecimal(BigDecimal val)
	{
		 ...
		 * 1234.56 will be normalized to 1.23456E3; 123.4E100 will be normalized
		 * to 1.234E102; -123.4E-100 will be normalized to -1.234E-98.
		 * 
		 * After the normalization, the byte format is: Byte 0: sign (-1
		 * represents negative, 0 represents zero, and 1 represents positive).
		 * Byte 1 ~ 5: the exponent with sign, and written as a
		 * SortedPackedInteger value. Byte 6 ~ N: the normalized decimal part
		 * with sign.
		 * 
		 * Get the scale and the unscaled value of this BigDecimal..
		 */
	...
	private final HGDataOutput writeSortedNormalizedBigDecimal(BigDecimal val)
	{
		 ...
		 * Byte 0 ~ N: Store all digits with sign. Each 9 digits is regarded as
		 * one integer, and written as a SortedPackedInteger value. If there are
		 * not enough 9 digits, pad trailing zeros. Since we may pad trailing
		 * zeros for serialization, when doing de-serialization, we need to
		 * delete the trailing zeros. In order to designate a special value as
		 * the terminator byte, we set val = (val < 0) ? (val - 1) : val. Byte N
		 * + 1: Terminator byte. The terminator byte is -1, and written as a
		...
		/*
		 * The default capacity of a StringBuilder is 16 chars, which is enough
		 * to hold a group of digits having 9 digits.
		 */
	...
	/**
	 * Returns the maximum byte length that would be output for a given
	 * {@code BigDecimal} value if {@link TupleOutput#writeSortedBigDecimal}
	 * were called.
	 * 
	 * @see <a href="package-summary.html#bigDecimalFormats">BigDecimal
	 *      Formats</a>
	...
	public static int getSortedBigDecimalMaxByteLength(BigDecimal val)
	{
		...
		return 1 /* sign */ + PackedInteger.MAX_LENGTH /* exponent */
	...
	{
		assert val <= 0x7FFFFFFFFFFFFFFL : "value is limited to 2^59"; //$NON-NLS-1$
			...
			else
			{
				...
				else
				{
					...
					else
					{
						...
						{
							writeFast((byte) (value & 0x7FL) | 0x80); // byte 7
																		// (51)
							value >>>= 7;
