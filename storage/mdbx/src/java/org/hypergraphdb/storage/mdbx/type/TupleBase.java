 ...
 *
 * <p>
 * Tuple bindings and key creators append data to a {@link TupleOutput}
 ...
 * {@link #setTupleBufferSize} method may be used to control the initial size of
 * the buffer, and the {@link #getTupleOutput} method may be overridden by
...
public class TupleBase<E>
{
	 ...
	 *
	 * <p>
	 * Unless {@link #setTupleBufferSize} is called, the default
	 * {@link com.sleepycat.util.FastOutputStream#DEFAULT_INIT_SIZE} size will
	 * be used.
	 * </p>
	 */
	public TupleBase()
	 ...
	 * {@link com.sleepycat.util.FastOutputStream#DEFAULT_INIT_SIZE} size is
	 * used.
	 * </p>
	 *
	 * @param byteSize
	 *            the initial byte size of the output buffer, or zero to use the
	 *            default size.
	 */
	 ...
	 * an initial buffer size that can be changed using the
	 * {@link #setTupleBufferSize} method.
	 * </p>
	 *
	 * <p>
	 ...
	 * {@link com.sleepycat.util.FastOutputStream#reset} method before each use.
	 * </p>
	 *
	 * @param object
	 *            is the object to be written to the tuple output, and may be
	...
	public static void outputToEntry(TupleOutput output, DatabaseEntry entry)
	{
		...
		toBuf = new byte[len];
		System.arraycopy(output.getBufferBytes(), 0, toBuf, 0, len);
		entry.setData(toBuf);
	}
	 ...
	 *
	 * @param input
	 *            is the source tuple input object.
	 *
	 * @param entry
	 *            is the destination entry buffer.
	 */
	public static void inputToEntry(TupleInput input, DatabaseEntry entry)
	{
		...
		toBuf = new byte[len];
		System.arraycopy(input.getBufferBytes(), 0, toBuf, 0, len);
		entry.setData(toBuf);
	}
