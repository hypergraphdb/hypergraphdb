...
public interface HGDataOutput
{
	...
	/**
	 * Encode and write a varint30. This is a special encoding supporting
	 * unsigned values up to 2^30 that are encoded in variable 2, 3 or 4 bytes
	 * representation. {@code value} is treated as unsigned, so it won't be
	 * sign-extended if negative.
	 */
	public void writeRawVarint30(final int val);

	/**
	 * Encode and write a varint59. This is a special encoding supporting
	 * unsigned values up to 2^59 that are encoded in variable 4 to 8 bytes
	 * representation. {@code value} is treated as unsigned, so it won't be
	 * sign-extended if negative.
	 */
