...
public class PackedInteger
{
	...
	public static int writeSortedInt(byte[] buf, int offset, int value)
	{
		...
		if (value < -119)
		{
			 ...
			 * we extract the significant byte from left to right.
			 *
			 * In the left to right order, if the first byte of the adjusted
			 * value is a significant byte, it will be stored in the 2nd byte of
			 * the buf. Then we will look at the 2nd byte of the adjusted value
			 * to see if this byte is the significant byte, if yes, this byte
			 * will be stored in the 3rd byte of the buf, and the like.
			 */
		...
		else if (value > 120)
		{
			 ...
			 * is the byte which is not equal to 0x00.
			 *
			 * In the left to right order, if the first byte of the adjusted
			 * value is a significant byte, it will be stored in the 2nd byte of
			 * the buf. Then we will look at the 2nd byte of the adjusted value
