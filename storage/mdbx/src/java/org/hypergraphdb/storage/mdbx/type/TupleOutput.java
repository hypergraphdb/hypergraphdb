...
/**
 * An <code>OutputStream</code> with <code>DataOutput</code>-like methods for writing tuple fields. It is used
 * by <code>TupleBinding</code>.
 *
 * <p>
 * This class has many methods that have the same signatures as methods in the {@link java.io.DataOutput}
 * interface. The reason this class does not implement {@link java.io.DataOutput} is because it would break
 * the interface contract for those methods because of data format differences.
 * </p>
 *
...
public class TupleOutput extends FastOutputStream {
	...
	/**
	 * Creates a tuple output object for writing a byte array of tuple data, using a given buffer. A new buffer
	...
	public final TupleOutput writeSortedBigDecimal(BigDecimal val) {
		...
		String unscaledValStr = unscaledVal.abs()
				.toString();
	...
	/**
	 * Returns the maximum byte length that would be output for a given {@code
	 * BigDecimal} value if {@link TupleOutput#writeSortedBigDecimal} were called.
	 *
	 * @see <a href="package-summary.html#bigDecimalFormats">BigDecimal Formats</a>
	 */
	public static int getSortedBigDecimalMaxByteLength(BigDecimal val) {
		String digitsStr = val.stripTrailingZeros()
				.unscaledValue()
				.abs()
				.toString();
