...
public class LmdbConfig
{
    ...
    private boolean syncForce = false;
  	...
  	static Properties getDefaultLmdbProperties() 
  	{
  		...
  		properties.setProperty("map.size", new Long(2000L * 1024 * 1024).toString()); // 2GB default
    ...
    public LmdbConfig()
    {
      lmdbProps = new Properties(getDefaultLmdbProperties()) {
		      ...
		      try {
		      	...
		      	else {  //converted possibly to class, package or whatever.
		      		newVal = val;
		      	}
					}
      ...
      final InputStream is = LmdbConfig.class.getResourceAsStream(propFileName);
  		...
  		if (is != null) {
    		try {
					...
					System.err.println("Error loading LMDB config properties. Will use built-in defaults ONLY");
					e.printStackTrace();
				}
  		}
  		else {
				System.err.println("Error finding LMDB config properties. Will use built-in defaults ONLY");
  		}
  		...
  		while (propertyNames.hasMoreElements()) {
				...
				if (key.startsWith("lmdb.")) {
					lmdbProps.setProperty(key.replaceAll("lmdb.", ""), sysProperties.getProperty(key));
				}
			}
